                ...
                <div class="max-w-xl">
                    <section>
                        ...
                        <form class="mt-6" id="formPeriksa"
                            action="{{ route('dokter.memeriksa.store', ['id' => $janjiPeriksa->id]) }}" method="POST">
                            @csrf
                            ...
                            <div class="mb-3 form-group">
                                ...
                                <textarea class="rounded form-control @error('catatan') is-invalid @enderror" id="catatan" name="catatan"
                                    rows="3" placeholder="Catatan Pemeriksaan">{{ old('catatan') }}</textarea>
                                @error('catatan')
                                    <div class="invalid-feedback text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                                ...
                                <select class="rounded form-control @error('obats') is-invalid @enderror" id="obats"
                                    ...
                                    @foreach ($obats as $obat)
                                        <option value="{{ $obat->id }}" data-harga="{{ $obat->harga }}"
                                            {{ collect(old('obats'))->contains($obat->id) ? 'selected' : '' }}>
                            ...
                            <input type="number"
                                ...
                                id="biaya_periksa" name="biaya_periksa" value="{{ old('biaya_periksa', 150000) }}"
